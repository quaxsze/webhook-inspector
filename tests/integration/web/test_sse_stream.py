...
async def test_sse_delivers_new_request_event(monkeypatch, database_url, engine, tmp_path):
    ...
    # Seed the notifier on app.state so the request-scoped dep can find it.
    sync_dsn = database_url.replace("+psycopg_async", "").replace("+psycopg", "")
    notifier = PostgresNotifier(dsn=sync_dsn)
    await notifier.start()
    app_service.state.notifier = notifier
